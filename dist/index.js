...
/***/ function(module, exports, __webpack_require__) {
	...
	var storedRedux = function storedRedux() {
	    var _this = this;

	    _classCallCheck(this, storedRedux);

	    this.start = function (store) {
	        return function (next) {
	            return function (action) {
	                var actionQueue = _this.getReStore().actions || [];
	                actionQueue.map(function (actionType) {
	                    console.log(actionType.type, store.getState());
	                    _this.devTools.send(actionType.type, store.getState());
	                });
	                actionQueue.push(next(action));

	                _this.reStore.setStore(store.getState(), 'actions', actionQueue);
	            };
	        };
	    };

	    this.getReStore = function () {
	        return JSON.parse(_this.reStore.getStore()) || {};
	    };

	    this.reStore = new _Storage2.default();
	    this.devTools = window.__REDUX_DEVTOOLS_EXTENSION__;
	};

	exports.default = storedRedux;
...
/***/ function(module, exports) {
	...
	var Storage = function Storage() {
	    ...
	    var storeKey = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : '__redux_store';

	    _classCallCheck(this, Storage);

	    this.initLocalStore = function () {
	        if (_this.storage.getItem(_this.storeKey)) return;

	        _this.storage.setItem(_this.storeKey, JSON.stringify({}));
	    };

	    this.setStore = function (currentState, type, actionArray) {
	        if (!currentState) return;
	        if (!type) {
	            _this.storage.setItem(_this.storeKey, JSON.stringify(currentState));
	        } else {
	            _this.storage.setItem(_this.storeKey, JSON.stringify(_extends({}, currentState, { actions: actionArray })));
	        }
	    };

	    this.getStore = function () {
	        return _this.storage.getItem(_this.storeKey);
	    };

	    this.clearStore = function () {
	        _this.storage.removeItem('__redux_store');
	    };
